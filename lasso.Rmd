...
d1 <- 0.02*l1_dist[m]
d2 <- 0.18*l1_dist[m]
for (j in c(80,60,50,40,30,25,20,15,10,8,6,4,3)) {
  xl <- paste("L1 distance: ", round(l1_dist[j], 2))
  plot(m1, xlim=c(0, 1.4*max(l1_dist)),type="n", xlab=xl)
  for (i in 1:n) {
    lines(l1_dist[1:m], m1$beta[i, 1:m], col="lightgray")
    lines(l1_dist[1:j], m1$beta[i, 1:j])
    if (abs(m1$beta[i,j]) > 0) {
      text(d1+l1_dist[j], m1$beta[i, j], names(mtcars[i+1]), adj=0)
      text(d2+l1_dist[j], m1$beta[i, j], signif(m1$beta[i, j], 2), adj=0)
    }
  }
}
```
